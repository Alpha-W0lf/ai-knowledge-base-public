...
- Configurable channel backfill (BACKFILL_DAYS default 7 days)
...
class NoSubtitlesError(SyncFailure):
    """Confirmed no usable English subtitle track available (terminal skip)."""


class RetryableSyncError(SyncFailure):
    """Transient or retryable failure (network, yt-dlp crash, rate limit, timeout)."""
...
class SyncState:
    ...
    def save(self):
        ...
        temp_fd, temp_path = tempfile.mkstemp(
            dir=self.state_file.parent, prefix="sync_state_", suffix=".tmp"
        )
        try:
...
def run_ytdlp(args: list[str], capture: bool = True, retry: bool = True) -> str | None:
    ...
    for attempt in range(attempts):
        try:
            ...
            if not capture:
                ...
                if "ERROR:" in stderr or "HTTP Error 4" in stderr:
                    if attempt < attempts - 1:
                        console.print("[yellow]yt-dlp error, retrying...[/yellow]")
            ...
            if attempt < attempts - 1:
                console.print(
                    f"[yellow]yt-dlp timeout, retrying in {config.RETRY_DELAY_SECONDS}s...[/yellow]"
                )
                time.sleep(config.RETRY_DELAY_SECONDS)
            else:
...
def convert_srt_to_md(srt_file: Path) -> Path | None:
    ...
    try:
        ...
        if not re.search(r"\d{2}:\d{2}:\d{2}", srt_content):
            console.print(
                f"[yellow]Warning: {srt_file.name} doesn't appear to be valid SRT[/yellow]"
            )
            return None
        ...
        temp_fd, temp_path = tempfile.mkstemp(
            dir=srt_file.parent, prefix="transcript_", suffix=".tmp"
        )
        try:
...
def download_transcript(video_id: str, output_dir: Path) -> Path:
    """
    Get transcript for a video - download if needed, convert SRT to markdown.

    Distinguishes success vs no_subs vs retryable failures:
    - Returns Path to MD file on success
    - Raises NoSubtitlesError when subtitles are confirmed missing
    - Raises RetryableSyncError on network/transient/conversion failures
    """
    ...
    if existing_srt and existing_srt.exists():
        ...
        md_file = convert_srt_to_md(existing_srt)
        if md_file and md_file.exists():
            return md_file
        raise RetryableSyncError(f"Failed to convert existing SRT: {existing_srt.name}")
    ...
    if ytdlp_res is None:
        raise RetryableSyncError(
            f"yt-dlp failed or timed out downloading subtitles for video {video_id}"
        )
    ...
    if not srt_file:
        raise NoSubtitlesError(f"No usable subtitles found for video {video_id}")

    md_file = convert_srt_to_md(srt_file)
    if not md_file or not md_file.exists():
        raise RetryableSyncError(f"Failed to convert downloaded SRT for video {video_id}")
...
def sync_channel(
    ...
    if last_sync and not force:
        ...
        if age < timedelta(days=config.SYNC_INTERVAL_DAYS):
            console.print(
                f"[dim]{channel}: synced {age.days}d {age.seconds // 3600}h ago, skipping[/dim]"
            )
            return stats
    ...
    for video in videos_to_process:
        try:
            ...
            # Confirmed: no subtitles available for this video
            console.print(f"  [yellow]⚠[/yellow] No transcript: {video['title'][:50]}...")
            no_subs_ids.append(video["id"])
            stats["no_subs"] += 1
        except Exception as e:
            # Retryable failure or exception during processing - will retry next time
...
def sync_all(force: bool = False) -> dict:
    ...
    console.print(
        f"[bold]Listing videos from {len(channels)} channels "
        f"({config.PARALLEL_WORKERS} workers)...[/bold]"
    )
    since_date = (datetime.now() - timedelta(days=config.BACKFILL_DAYS)).strftime("%Y%m%d")
    ...
    for channel, description in channels:
        ...
        if not videos_to_process:
            console.print("  [dim]All have no subtitles[/dim]")
            state.update(channel)
            continue
                ...
                if result_md and result_md.exists():
                    ...
                    try:
                        ...
                        if chunks > 0:
                            if not quiet:
                                console.print(
                                    f"    [green]✓[/green] Converted and ingested ({chunks} chunks)"
                                )
                            stats["ingested"] += 1
                        else:
                            if not quiet:
                                console.print("    [yellow]⚠[/yellow] Converted but no chunks")
                    except Exception as e:
                        if not quiet:
...
def main(force: bool, status: bool, add: str, channel: str):
    ...
    if add:
        ...
        console.print(f'Add: {{"handle": "{add}", "description": "description here"}}')
        return
